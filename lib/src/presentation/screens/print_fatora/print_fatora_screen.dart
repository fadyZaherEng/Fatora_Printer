            ...
            child: Column(
              children: [
                RepaintBoundary(
                  ...
                  child: _buildFactoraDetails(isPrint:false),
                ),
...
// Return the image data as Uint8List
  ...
  void _connectWithBluetoothPrinter() async {
    if (await PermissionServiceHandler().handleServicePermission(
      ...
      // // Update the state with the captured image
      // _imageBytes = pngBytes; // Set the image bytes to display later
      _imageBytes=await createImageFromWidget(_buildFactoraDetails(isPrint: true), logicalSize: const Size(500, 500),
  ...
  Widget _buildFactoraDetails({bool isPrint = false}) {
    return Container(
      // color: Colors.white,
      child: Padding(
